# КАК ИСПОЛЬЗОВАТЬ СОХРАНЕННЫЕ МОДЕЛИ И ДЕЛАТЬ ПРОГНОЗ НА БУДУЩЕЕ

## 📋 Содержание

1. [Формат входного файла](#0-формат-входного-файла)
2. [Использование простого режима Streamlit (РЕКОМЕНДУЕТСЯ)](#1-использование-простого-режима-streamlit)
3. [Обучение модели на всех данных (для продакшн-прогноза)](#2-обучение-модели-на-всех-данных)
4. [Прогноз на будущее от последней даты](#3-прогноз-на-будущее)
5. [Дообучение модели на новых данных](#4-дообучение-модели)
6. [Использование сохраненной модели](#5-использование-сохраненной-модели)
7. [Нужно ли переобучать при смене датасета?](#6-нужно-ли-переобучать)

---

## 0. Формат входного файла

### Расширение и кодировка

- **Расширение:** `.csv`
- **Формат:** CSV (Comma-Separated Values)
- **Кодировка:** UTF-8 (рекомендуется) или latin-1
- **Разделитель:** запятая (`,`)

### Требования к именованию полей

⚠️ **ВАЖНО:** Используйте формат **snake_case** для имен полей (например, `product_id`, а не `ProductID`).

### Поля входного файла

Система поддерживает два формата: **новый (рекомендуемый)** и **старый (устаревший)**.

#### ✅ Новый формат (рекомендуется)

Используйте этот формат для новых проектов.

**Обязательные поля:**
- `order_date` — дата заказа (формат: YYYY-MM-DD или другой распознаваемый pandas формат)
- `category` — категория товара (строка)
- `price` — цена единицы товара (число, >= 0)
- `qty_ordered` — количество проданных единиц (число, >= 0)

**Рекомендованные поля:**
- `product_id` (или `item_id`, или `sku`) — уникальный идентификатор товара (строка/число)
  - Необходим для агрегации по продуктам
  - Если не указан, будет заменен на 'unknown'
- `discount` (или `discount_amount`) — размер скидки в денежных единицах (число, >= 0)
  - Необходим для использования регрессоров (`include_regressors=True`)
  - Если не указан, будет установлен в 0
- `status` — статус заказа (строка)
  - Используется для фильтрации: только заказы со статусом `'complete'` или `'received'` учитываются
  - Если не указан, все заказы учитываются

**Пример структуры файла:**

```csv
order_date,category,price,qty_ordered,product_id,discount,status
2024-01-01,Electronics,1500.50,2,PROD-001,100.0,complete
2024-01-01,Clothing,850.00,1,PROD-002,50.0,complete
2024-01-02,Electronics,1500.50,3,PROD-001,150.0,received
2024-01-02,Home & Garden,2500.00,1,PROD-003,0.0,complete
```

#### ⚠️ Старый формат (устаревший, но поддерживается)

Используется для обратной совместимости с существующими данными.

**Обязательные поля:**
- `Sale_Date` — дата продажи
- `Product_ID` — идентификатор товара
- `Product_Category` — категория товара
- `Unit_Price` — цена единицы товара
- `Discount` — размер скидки
- `Quantity_Sold` — количество проданных единиц

**Примечание:** Старый формат автоматически преобразуется в новый формат внутри системы.

### Маппинг полей

Система автоматически преобразует поля в внутренний формат:

| Входное поле (новый формат) | Внутреннее поле | Назначение |
|----------------------------|-----------------|------------|
| `order_date` | `ds` | Дата (временной ряд) |
| `product_id` / `item_id` / `sku` | `product_id` | Идентификатор товара |
| `category` | `category` | Категория товара |
| `price` | `price` | Цена единицы |
| `discount` / `discount_amount` | `discount` | Размер скидки |
| `qty_ordered` | `y` | Целевая переменная (количество) |

### Минимальные требования

- **Минимум строк:** 100 (рекомендуется для стабильной работы модели)
- **Минимум уникальных дат:** зависит от данных (для daily aggregation нужна хотя бы одна дата)
- **Валидные даты:** все значения в `order_date` должны быть корректными датами
- **Непустые значения:** `order_date` и `qty_ordered` не могут быть пустыми (NaN значения удаляются)

### Проверка файла перед использованием

Используйте скрипт для проверки файла:

```python
python scripts/check_csv.py path/to/your/file.csv
```

### Пример валидного файла

```csv
order_date,category,price,qty_ordered,product_id,discount,status
2024-01-01,Electronics,1500.50,2,PROD-001,100.0,complete
2024-01-01,Clothing,850.00,1,PROD-002,50.0,complete
2024-01-02,Electronics,1500.50,3,PROD-001,150.0,received
2024-01-02,Home & Garden,2500.00,1,PROD-003,0.0,complete
2024-01-03,Electronics,1200.00,5,PROD-004,0.0,complete
```

---

## 1. Использование простого режима Streamlit (РЕКОМЕНДУЕТСЯ)

### Для быстрого получения прогноза

**Простой режим** автоматизирует весь процесс и использует оптимальные настройки:

1. **Запустите Streamlit:**
   ```bash
   streamlit run streamlit_app/app.py
   ```

2. **Выберите "Simple" режим** в боковой панели

3. **Загрузите CSV файл** с данными о продажах

4. **Нажмите "🚀 Обучить модель (автоматически)"**
   - Система автоматически:
     - Загрузит файл в backend
     - Выполнит предобработку
     - Обучит shop-level модель с оптимальными параметрами:
       - ❌ Без регрессоров (для простоты)
       - ✅ С log-transform (для лучшего качества)
       - ✅ С auto-tune (автоматический подбор параметров)
       - ✅ На всех данных (skip_holdout=True)

5. **Нажмите "📊 Получить прогноз по всем категориям"**
   - Укажите горизонт прогноза (по умолчанию 30 дней)
   - Система автоматически:
     - Сгенерирует shop-level прогноз с log-transform
     - Распределит прогноз по всем категориям
     - Покажет отдельные графики для каждой категории

**Результат:** Прогноз для всех категорий с визуализацией.

**Преимущества простого режима:**
- ✅ Не требует настройки параметров
- ✅ Использует оптимальные настройки по умолчанию
- ✅ Распределенный прогноз категорий (более точный)
- ✅ Автоматическая визуализация для всех категорий

---

## 2. Обучение модели на всех данных

### Проблема
По умолчанию модель делит данные на train/test (80%/20%), поэтому прогноз делается на **уже известные** данные (тестовый период).

### Решение
Используйте параметр `skip_holdout=True` при обучении - модель обучится на **ВСЕХ** данных без разделения.

### Через API:
```python
POST /train
{
    "shop_csv": "data/processed/test_shop.csv",
    "model_out": "models/production_model.pkl",
    "skip_holdout": true,  # ← НОВЫЙ ПАРАМЕТР!
    "include_regressors": true,
    "holdout_frac": 0.2  # Игнорируется если skip_holdout=True
}
```

### Через код:
```python
from app.train import train_prophet

result = train_prophet(
    shop_csv_path="data/processed/test_shop.csv",
    model_out_path="models/production_model.pkl",
    skip_holdout=True,  # ← Использует ВСЕ данные
    include_regressors=True
)
```

### Важно:
- ✅ Модель обучится на **всех** данных
- ⚠️ Метрики (MAPE, MAE) **не будут вычислены** (нет тестового набора)
- ✅ Модель готова для **прогноза на будущее**

---

## 3. Прогноз на будущее

### Как работает сейчас

Функция `predict_prophet()` автоматически делает прогноз от **последней даты в истории модели**:

```python
from app.predict import predict_prophet

# Загружает модель и делает прогноз на horizon_days дней вперед
forecast = predict_prophet(
    model_path="models/production_model.pkl",
    horizon_days=30,  # Прогноз на 30 дней вперед
    last_known_regressors_csv="data/processed/test_shop.csv"  # Для регрессоров
)
```

**Результат:** Прогноз на даты **после** последней даты в истории модели.

### Через API:
```python
POST /predict
{
    "model_path": "models/production_model.pkl",
    "horizon": 30,
    "last_known_regressors_csv": "data/processed/test_shop.csv",
    "smooth_transition": true
}
```

### Пример: Прогноз на следующий месяц

1. **У вас есть данные до 2025-01-15**
2. **Нужен прогноз на февраль 2025**

```python
# Шаг 1: Обучение на всех данных (если еще не обучена)
from app.train import train_prophet

train_prophet(
    shop_csv_path="data/processed/all_data_until_2025-01-15.csv",
    model_out_path="models/model_2025-01-15.pkl",
    skip_holdout=True  # ВСЕ данные для обучения
)

# Шаг 2: Прогноз на 30 дней вперед
from app.predict import predict_prophet

forecast = predict_prophet(
    model_path="models/model_2025-01-15.pkl",
    horizon_days=30,
    last_known_regressors_csv="data/processed/all_data_until_2025-01-15.csv"
)

# Результат: прогноз с 2025-01-16 по 2025-02-15
print(forecast[['ds', 'yhat']])
```

---

## 4. Дообучение модели на новых данных

### Когда использовать

Когда у вас:
- ✅ Есть уже обученная модель
- ✅ Появились новые данные (например, за январь 2025)
- ✅ Нужно обновить модель БЕЗ потери старой истории

### Решение: Модуль `app/update_model.py`

```python
from app.update_model import update_model_with_new_data

# Дообучение модели на новых данных
result = update_model_with_new_data(
    model_path="models/old_model.pkl",
    new_data_csv="data/processed/new_data_january_2025.csv",
    output_model_path="models/updated_model.pkl",  # Или None чтобы перезаписать
    include_regressors=True,
    max_history_days=None  # None = использовать всю историю
)

print(f"Обновлено: {result['combined_data_days']} дней данных")
print(f"Период: {result['date_range']['start']} to {result['date_range']['end']}")
```

### Как это работает:

1. Загружает старую модель и её историю
2. Объединяет старые данные + новые данные
3. Переобучает модель на объединенном датасете
4. Сохраняет обновленную модель

### Важно:
- ✅ Сохраняет всю историю (старую + новую)
- ✅ Автоматически определяет параметры старой модели
- ✅ Может ограничить историю (`max_history_days`) для производительности

---

## 5. Использование сохраненной модели

### Способ 1: Прямой прогноз

```python
from app.predict import predict_prophet

# Загрузите сохраненную модель и сделайте прогноз
forecast = predict_prophet(
    model_path="models/my_model.pkl",
    horizon_days=30
)
```

### Способ 2: Через FastAPI

```python
import requests

response = requests.post(
    "http://localhost:8000/predict",
    json={
        "model_path": "models/my_model.pkl",
        "horizon": 30,
        "last_known_regressors_csv": "data/processed/test_shop.csv"
    }
)

forecast = response.json()
```

### Способ 3: Через Streamlit UI

#### Простой режим (РЕКОМЕНДУЕТСЯ):

1. Выберите **"Simple"** режим в боковой панели
2. Загрузите CSV файл
3. Нажмите **"🚀 Обучить модель (автоматически)"**
   - Система автоматически выполнит все шаги с оптимальными настройками
4. После обучения нажмите **"📊 Получить прогноз по всем категориям"**
   - Укажите горизонт прогноза (по умолчанию 30 дней)
   - Результат: прогноз для всех категорий с визуализацией

#### Продвинутый режим:

1. Выберите **"Advanced"** режим в боковой панели
2. Загрузите данные
3. Обучите модель (или используйте существующую)
4. В разделе "🔮 Шаг 5: Генерация прогноза" укажите:
   - Модель: путь к `.pkl` файлу
   - Horizon: количество дней
   - Log-transform: должно совпадать с обучением
   - Нажмите "🔮 Сгенерировать прогноз"
5. Для категорий: используйте раздел "Распределенный прогноз категорий"

---

## 6. Нужно ли переобучать при смене датасета?

### Ответ: Зависит от ситуации

#### ✅ НЕ нужно переобучать, если:

1. **Просто делаете прогноз на будущее**
   - Модель уже обучена на исторических данных
   - Нужен прогноз на следующие 30 дней
   - **Решение:** Просто используйте `predict_prophet()` с существующей моделью

2. **Новые данные поступают постепенно (streaming)**
   - Каждый день приходят новые данные
   - Модель нужно обновлять периодически (раз в неделю/месяц)
   - **Решение:** Используйте `update_model_with_new_data()` раз в неделю/месяц

#### ✅ НУЖНО переобучать, если:

1. **Полностью новый датасет (другой магазин/период)**
   - Данные из другого источника
   - Другая динамика продаж
   - **Решение:** Обучение с нуля через `train_prophet()`

2. **Значительные изменения в данных**
   - Резкое изменение тренда
   - Новые сезонные паттерны
   - **Решение:** Переобучение с нуля

3. **Накопилось много новых данных (>30% от исходных)**
   - Было 365 дней, стало 500 дней
   - **Решение:** Дообучение через `update_model_with_new_data()`

### Рекомендации:

| Ситуация | Решение | Частота |
|----------|---------|---------|
| Ежедневный прогноз | Использовать сохраненную модель | Каждый день |
| Новые данные 1-2 недели | Дообучение модели | Раз в месяц |
| Новые данные >1 месяц | Дообучение или переобучение | По необходимости |
| Полностью новый датасет | Обучение с нуля | Один раз |

---

## 📝 Примеры использования

### Пример 1: Ежедневный прогноз

```python
# У вас есть модель, обученная на данных до 2025-01-15
# Каждый день вам нужен прогноз на следующие 7 дней

from app.predict import predict_prophet

daily_forecast = predict_prophet(
    model_path="models/production_model.pkl",
    horizon_days=7,
    smooth_transition=True
)
```

### Пример 2: Обновление модели раз в месяц

```python
# В конце месяца обновляете модель на новых данных

from app.update_model import update_model_with_new_data

# Новые данные за январь
update_model_with_new_data(
    model_path="models/model_2024-12.pkl",
    new_data_csv="data/processed/january_2025.csv",
    output_model_path="models/model_2025-01.pkl"
)

# Теперь модель обучена на всех данных до конца января
```

### Пример 3: Полное переобучение на новом датасете

```python
from app.train import train_prophet

# Новый датасет - обучаем с нуля
train_prophet(
    shop_csv_path="data/processed/new_shop_2025.csv",
    model_out_path="models/new_shop_model.pkl",
    skip_holdout=True,  # Используем все данные
    include_regressors=True,
    auto_tune=True  # Опционально: автоматический подбор параметров
)
```

---

## 🎯 Краткая инструкция

### Для быстрого прогноза (РЕКОМЕНДУЕТСЯ):

**Используйте простой режим Streamlit:**
1. ✅ Запустите Streamlit: `streamlit run streamlit_app/app.py`
2. ✅ Выберите "Simple" режим
3. ✅ Загрузите CSV файл
4. ✅ Нажмите "🚀 Обучить модель (автоматически)"
5. ✅ Нажмите "📊 Получить прогноз по всем категориям"
6. ✅ Результат: прогноз для всех категорий с визуализацией

### Для прогноза на будущее (через код/API):

1. ✅ Обучите модель с `skip_holdout=True` (на всех данных)
2. ✅ Используйте `predict_prophet()` для прогноза
3. ✅ Результат: прогноз на даты **после** последней даты в данных

**В простом режиме:** `skip_holdout=True` включено по умолчанию.

### При появлении новых данных:

1. ✅ Если немного новых данных (< 30%): используйте `update_model_with_new_data()`
2. ✅ Если много новых данных (> 30%) или другой датасет: переобучение с нуля

### Использование сохраненных моделей:

1. ✅ **Простой режим:** Просто загрузите CSV и обучите модель
2. ✅ **Продвинутый режим:** Укажите путь к `.pkl` файлу в разделе "Generate Forecast"
3. ✅ **Через код:** Укажите путь к `.pkl` файлу в `predict_prophet()`

---

## ❓ FAQ

**Q: Модель обучена до 2024-12-31. Как сделать прогноз на январь 2025?**
A: Используйте `predict_prophet()` - она автоматически сделает прогноз от последней даты модели на указанное количество дней вперед.

**Q: Появились новые данные за январь. Что делать?**
A: Используйте `update_model_with_new_data()` для дообучения на новых данных, сохраняя всю историю.

**Q: Нужно ли переобучать каждый раз при новых данных?**
A: Нет! Для ежедневного прогноза просто используйте сохраненную модель. Обновляйте раз в неделю/месяц.

**Q: Как узнать, когда модель устарела?**
A: Проверяйте качество прогноза. Если метрики ухудшились на 20%+ - время обновить модель.

**Q: Какой режим использовать в Streamlit?**
A: **Простой режим (Simple)** рекомендуется для большинства случаев. Он автоматизирует весь процесс и использует оптимальные настройки. Продвинутый режим используйте только если нужен полный контроль над параметрами.

**Q: Какие настройки используются в простом режиме?**
A: 
- ❌ Регрессоры: отключены (для простоты)
- ✅ Log-transform: включен (для лучшего качества)
- ✅ Auto-tune: включен (автоматический подбор параметров)
- ✅ Skip holdout: включен (обучение на всех данных)
- ✅ Распределенный прогноз категорий: используется по умолчанию
- ✅ Показываются все категории: по умолчанию (не только топ-5)

